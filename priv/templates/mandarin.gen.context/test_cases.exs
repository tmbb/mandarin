  ...
  describe "<%= schema.plural %>" do
    ...
    test "list_all_<%= schema.plural %>/0 returns all <%= schema.plural %>" do
      <%= schema.singular %> = <%= schema.singular %>_fixture()
      assert <%= inspect context.alias %>.list_all_<%= schema.plural %>() == [<%= schema.singular %>]
    end

    test "list_<%= schema.plural %>/1 returns paginated results" do
      <%= schema.singular %> = <%= schema.singular %>_fixture()
      {:ok, {results, _meta}} = <%= inspect context.alias %>.list_<%= schema.plural %>(%{})
      assert results == [<%= schema.singular %>]
    end
