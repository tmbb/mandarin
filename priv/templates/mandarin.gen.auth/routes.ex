  ...
  scope <%= router_scope %> do
    pipe_through [:browser, :<%= context.basename %>_layout, :redirect_if_<%= schema.singular %>_is_authenticated]<%= if live? do %>
  ...
  scope <%= router_scope %> do
    pipe_through [:browser, :<%= context.basename %>_layout, :require_authenticated_<%= schema.singular %>]<%= if live? do %>
