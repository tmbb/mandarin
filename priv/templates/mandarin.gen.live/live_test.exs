defmodule <%= inspect context.web_module %>.<%= inspect Module.concat(schema.web_namespace, schema.alias) %>LiveTest do
  ...
  import Mandarin.LiveViewTest
  import <%= inspect context.module %>Fixtures

  @create_attrs <%= Mix.Mandarin.to_text for {key, value} <- schema.params.create, into: %{}, do: {key, Mix.Mandarin.Schema.live_form_value(value)} %>
  @update_attrs <%= Mix.Mandarin.to_text for {key, value} <- schema.params.update, into: %{}, do: {key, Mix.Mandarin.Schema.live_form_value(value)} %>
  @invalid_attrs <%= Mix.Mandarin.to_text for {key, value} <- schema.params.create, into: %{}, do: {key, value |> Mix.Mandarin.Schema.live_form_value() |> Mix.Mandarin.Schema.invalid_form_value()} %>
  ...
  describe "Index" do
    ...
    test "saves new <%= schema.singular %>", %{conn: conn} do
      ...
      assert index_live
             ...
             |> render_change() =~ "<%= Mix.Mandarin.Schema.failed_render_change_message(schema) %>"
