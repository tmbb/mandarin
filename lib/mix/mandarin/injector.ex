defmodule Mix.Mandarin.Injector do
  @moduledoc false

  alias Mix.Mandarin.{Context, Schema}
  alias Mix.Tasks.Mandarin.Gen.Auth.HashingLibrary
  ...
  def inject_after(code, marker, code_to_inject) do
    code = to_unix_line_endings(code)
    case String.split(code, marker <> "\n", parts: 2) do
      [before_marker, after_marker] ->
        {:ok, before_marker <> marker <> "\n" <> code_to_inject <> after_marker}
  ...
  def inject_before(code, marker, code_to_inject) do
