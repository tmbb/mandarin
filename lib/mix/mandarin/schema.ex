defmodule Mix.Mandarin.Schema do
  @moduledoc false
  ...
  def new(schema_name, schema_plural, cli_attrs, opts) do
    ...
    base = Mix.Mandarin.context_base(ctx_app)
    basename = Mandarin.Naming.underscore(schema_name)
    module = Module.concat([base, schema_name])
    repo = opts[:repo] || Module.concat([base, "Repo"])
    file = Mix.Mandarin.context_lib_path(ctx_app, basename <> ".ex")
    ...
    types = types(attrs)
    web_namespace = opts[:web] && Mandarin.Naming.camelize(opts[:web])
    ...
    search_field = default_search_field(attrs)
  ...
  """
  def default_search_field(attrs) do
    case Enum.find(attrs, fn {_field, type} -> type in [:string, :text] end) do
      {field, _type} -> field
      nil -> nil
    end
  end
  ...
  * In case it is a boolean, we keep it as false
  ...
  def attrs_to_cast(schema) do
    Enum.map_join(schema.attrs ++ schema.assocs, ", ", fn params ->
      case params do
        {k, _v} -> inspect(k)
        {_, k, _, _} -> inspect(k) <> "_id"
      end
    end)
  end

  def format_fields_for_schema(schema) do
    all_field = Enum.into(schema.types, []) ++ schema.assocs

    all_field
    |> Enum.reject(&is_foreign_id?/1)
    |> Enum.map(fn params -> format_field_for_schema(schema, params) end)
    |> Enum.join("\n")
  end

  def is_foreign_id?({k, _v}), do: String.ends_with?(to_string(k), "_id")
  def is_foreign_id?(_), do: false

  defp format_field_for_schema(schema, parameters) do
    case parameters do
      {_k, _v} -> format_simple_field_for_schema(schema, parameters)
      {_, _k, _alias, _t} -> format_assoc_for_schema(schema, parameters)
    end
  end

  defp format_simple_field_for_schema(schema, {k, v}) do
    "    field #{inspect(k)}, #{type_and_opts_for_schema(v)}#{schema.defaults[k]}#{maybe_redact_field(k in schema.redacts)}"
  end

  defp format_assoc_for_schema(_schema, {_, k, string_alias, _table_name_atom}) do
    module_alias = string_alias |> to_string() |> String.split(".") |> Enum.at(-1)
    k_without_id = String.trim_trailing(to_string(k), "_id")
    "    belongs_to :#{k_without_id}, #{module_alias}, on_replace: :nilify"
  end

  def assoc_aliases(schema) do
    case schema.assocs do
      [] ->
        # Return the empty string
        ""

      _other ->
        # Empty string surrounded by newlines
        schema.assocs
        |> Enum.map(fn {_, _k, string_alias, _t} -> "\n  alias #{string_alias}" end)
        |> then(fn lines -> [lines, "\n"] end)
    end
  end
  ...
  For example:
      ...
      mix mandarin.gen.schema Comment comments body:text status:enum:published:unpublished
  """
  ...
  defp validate_attr!({name, :array}) do
    ...
    For example:
        ...
        mix mandarin.gen.schema Post posts settings:array:string
    """)
  end
  ...
  defp partition_attrs_and_assocs(schema_module, attrs) do
      ...
      Enum.split_with(attrs, fn
        ...
        {key, :references} ->
          ...
          For example:
              ...
              mix mandarin.gen.schema Comment comments body:text post_id:references:posts
          """)
    ...
    assocs =
      Enum.map(assocs, fn {key_id, {:references, source}} ->
        ...
        module = Module.concat(base ++ [Mandarin.Naming.camelize(key)])
  ...
  defp indexes(table, assocs, uniques) do
    ...
    |> Enum.map(fn
      {key, false} -> "create index(:#{table}, [:#{key}_id])"
      {key, true} -> "create unique_index(:#{table}, [:#{key}_id])"
    end)
  end
