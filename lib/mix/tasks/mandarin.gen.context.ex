defmodule Mix.Tasks.Mandarin.Gen.Context do
  ...
  Generates a context with functions around an Ecto schema.
      ...
      mix mandarin.gen.context Admin User users name:string age:integer
  ...
  Overall, this generator will add the following files to `lib/your_app`:
    ...
    * a context module in `accounts/admin.ex`, serving as the API boundary
    * a schema in `admin/user.ex`, with a `users` table
  ...
  a `--table` option may be provided. For example:
      ...
      mix mandarin.gen.context Admin User users --table cms_users
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise("mix mandarin.gen.context can only be run inside an application directory")
    end
  ...
  defp inject_tests(%Context{test_file: test_file} = context, paths, binding) do
    unless Context.pre_existing_tests?(context) do
      Mix.Generator.create_file(
        ...
        Mix.Mandarin.eval_from(
          paths,
          "priv/templates/mandarin.gen.context/context_test.exs",
          binding
        )
      )
    end
  ...
  def raise_with_help(msg) do
    ...
    mix mandarin.gen.context expects a context module name,
    followed by singular and plural names of the generated resource,
    ending with any number of attributes.
    For example:

        mix mandarin.gen.context Accounts User users name:string --binary-id
