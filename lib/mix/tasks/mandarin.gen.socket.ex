defmodule Mix.Tasks.Mandarin.Gen.Socket do
  ...
  You can then generated channels with `mix mandarin.gen.channel`.
  """
  use Mix.Task
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix mandarin.gen.socket must be invoked from within your *_web application root directory"
      )
    end
    ...
    binding = Mix.Mandarin.inflect(socket_name)

    existing_channel =
      if pre_existing_channel do
        channel_binding = Mix.Mandarin.inflect(pre_existing_channel)
    ...
    Mix.Mandarin.check_module_name_availability!(binding[:module] <> "Socket")

    Mix.Mandarin.copy_from(paths(), "priv/templates/mandarin.gen.socket", binding, [
    ...
    Add the socket handler to your `#{Mix.Mandarin.web_path(context_app, "endpoint.ex")}`, for example:
  ...
  defp raise_with_help do
    Mix.raise("""
    mix mandarin.gen.socket expects the module name:
