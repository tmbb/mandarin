defmodule Mix.Tasks.Bureaucrat.Install do
  ...
  def run(args) do
    ...
    main_header_html_path = Path.join([web_prefix, "templates", layout_dir, "main-header.html.eex"])
    sidebar_html_path = Path.join([web_prefix, "templates", layout_dir, "sidebar.html.eex"])
    layout_view_path = Path.join([web_prefix, "views", "#{install.context_underscore}_layout_view.ex"])
  ...
  EEx.function_from_file(:defp, :layout_html, "priv/templates/bureaucrat.install/layout.html.eex", [:install])
  EEx.function_from_file(:defp, :main_header_html, "priv/templates/bureaucrat.install/main-header.html.eex", [:_install])
  EEx.function_from_file(:defp, :sidebar_html, "priv/templates/bureaucrat.install/sidebar.html.eex", [:_install])
  EEx.function_from_file(:defp, :layout_view, "priv/templates/bureaucrat.install/layout_view.ex", [:install])

  @doc false
  def print_shell_instructions(_opts) do
    nil
  end
end
