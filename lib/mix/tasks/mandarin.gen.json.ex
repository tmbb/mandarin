defmodule Mix.Tasks.Mandarin.Gen.Json do
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix mandarin.gen.json must be invoked from within your *_web application root directory"
      )
    end
  ...
  def files_to_be_generated(%Context{schema: schema, context_app: context_app}) do
    ...
    web = Mix.Mandarin.web_path(context_app)
  ...
  def print_shell_instructions(%Context{schema: schema, context_app: ctx_app} = context) do
    if schema.web_namespace do
      ...
      Add the resource to your #{schema.web_namespace} :api scope in #{Mix.Mandarin.web_path(ctx_app)}/router.ex:
