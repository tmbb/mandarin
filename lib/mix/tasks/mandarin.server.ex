defmodule Mix.Tasks.Mandarin.Server do
  use Mix.Task
  ...
  `server: true` in your `Mandarin.Endpoint` configuration.
  ...
  For example, to run `mandarin.server` without recompiling:

      $ mix mandarin.server --no-compile

  The `--no-halt` flag is automatically added.

  Note that the `--no-deps-check` flag cannot be used this way,
  because Mix needs to check dependencies to find `mandarin.server`.

  To run `mandarin.server` without checking dependencies, you can run:

      $ mix do deps.loadpaths --no-deps-check, mandarin.server
  """
