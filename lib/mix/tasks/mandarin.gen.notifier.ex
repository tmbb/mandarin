defmodule Mix.Tasks.Mandarin.Gen.Notifier do
  @shortdoc "Generates a notifier that delivers emails by default"

  @moduledoc """
  Generates a notifier that delivers emails by default.

      $ mix mandarin.gen.notifier Accounts User welcome_user reset_password confirmation_instructions
  ...
  generated in a different app under an umbrella.
      ...
      $ mix mandarin.gen.notifier Accounts User welcome_user --context-app marketing
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix mandarin.gen.notifier must be invoked from within your *_web application root directory"
      )
    end

    {context, notifier_module, messages} = build(args)

    inflections = Mix.Mandarin.inflect(notifier_module)
  ...
  def raise_with_help(msg) do
    ...
    mix mandarin.gen.notifier expects a context module name, followed by a
    ...
    For example:
        ...
        mix mandarin.gen.notifier Accounts User welcome reset_password
  ...
  defp copy_new_files(%Context{} = context, binding, paths) do
    ...
    Mix.Mandarin.copy_from(paths, "priv/templates/mandarin.gen.notifier", binding, files)
