defmodule Mix.Tasks.Bureaucrat.Gen.Schema do
  @shortdoc "Generates an Ecto schema and migration file"

  @moduledoc """
  Generates an Ecto schema and migration.

      mix bureaucrat.gen.schema Blog.Post blog_posts title:string views:integer
  ...
  the CLI.
      ...
      # Via CLI
      mix bureaucrat.gen.schema Blog.Post blog_posts title:string views:integer --context-app marketing
  ...
  the type makes it default to `:string`:
      ...
      mix bureaucrat.gen.schema Blog.Post blog_posts title views:integer
  ...
  referenced table:
      ...
      mix bureaucrat.gen.schema Blog.Post blog_posts title user_id:references:users
  ...
  type of the underlying array element to be given too:
      ...
      mix bureaucrat.gen.schema Blog.Post blog_posts tags:array:string

  Unique columns can be automatically generated by using:

      mix bureaucrat.gen.schema Blog.Post blog_posts title:unique unique_int:integer:unique
  ...
  a `--table` option may be provided. For example:
      ...
      mix bureaucrat.gen.schema Blog.Post posts --table cms_posts
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise "mix bureaucrat.gen.schema can only be run inside an application directory"
    end
  ...
  def copy_new_files(%Schema{context_app: ctx_app} = schema, paths, binding) do
    ...
    Mix.Bureaucrat.copy_from(paths, "priv/templates/bureaucrat.gen.schema", binding, files)

    if schema.migration? do
      migration_path = Mix.Bureaucrat.context_app_path(ctx_app, "priv/repo/migrations/#{timestamp()}_create_#{schema.table}.exs")
      Mix.Bureaucrat.copy_from paths, "priv/templates/bureaucrat.gen.schema", binding, [
  ...
  def validate_args!([schema, plural | _] = args, help) do
    cond do
      ...
      String.contains?(plural, ":") or plural != Bureaucrat.Naming.underscore(plural) ->
  ...
  def raise_with_help(msg) do
    ...
    mix bureaucrat.gen.schema expects both a module name and
    the plural of the generated resource followed by
    any number of attributes:

        mix bureaucrat.gen.schema Blog.Post blog_posts title:string
    """
  end
