defmodule Mix.Tasks.Mandarin.Gen.Auth do
  ...
  [`mix mandarin.gen.auth` guide](mix_phx_gen_auth.html).
  ...
  workflows:
      ...
      $ mix mandarin.gen.auth Store User users
      $ mix mandarin.gen.auth Backoffice Admin admins
  ...
  defp validate_required_dependencies! do
    unless Code.ensure_loaded?(Ecto.Adapters.SQL) do
      raise_with_help("mix mandarin.gen.auth requires ecto_sql", :phx_generator_args)
    end

    if generated_with_no_html?() do
      raise_with_help("mix mandarin.gen.auth requires phoenix_html", :phx_generator_args)
    end
  end
  ...
  defp files_to_be_generated(%Context{schema: schema, context_app: context_app} = context) do
    ...
    web_pre = Mix.Mandarin.web_path(context_app)
    web_test_pre = Mix.Mandarin.web_test_path(context_app)
    migrations_pre = Mix.Mandarin.context_app_path(context_app, "priv/repo/migrations")
    web_path = to_string(schema.web_path)
    web_context_pre = Path.join([web_pre, context.basename, singular])
    ...
    default_files = [
      ...
      "auth.ex": [web_pre, context.basename, "#{singular}_auth.ex"],
      "auth_test.exs": [web_test_pre, web_path, "#{singular}_auth_test.exs"],
      "session_controller.ex": [web_context_pre, "#{singular}_session_controller.ex"],
    ...
    non_live_files = [
      "confirmation_html.ex": [web_context_pre, "#{singular}_confirmation_html.ex"],
      ...
      "confirmation_controller.ex": [web_context_pre, "#{singular}_confirmation_controller.ex"],
      ...
      "registration_controller.ex": [web_context_pre, "#{singular}_registration_controller.ex"],
      ...
      "registration_html.ex": [web_context_pre, "#{singular}_registration_html.ex"],
      "reset_password_html.ex": [web_context_pre, "#{singular}_reset_password_html.ex"],
      ...
      "session_html.ex": [web_context_pre, "#{singular}_session_html.ex"],
      "session_new.html.heex": [web_context_pre, "#{singular}_session_html", "new.html.heex"],
      "settings_html.ex": [web_context_pre, "#{singular}_settings_html.ex"],
      "settings_controller.ex": [web_context_pre, "#{singular}_settings_controller.ex"],
  ...
  # def inject_on_mount_macro(context, file) do
  #   module = Module.concat([
  #     context.web_module,
  #     context.alias,
  #     inspect(context.schema.alias) <> "Auth"
  #   ])

  #   macro = """

  #     defmacro assign_user_on_mount() do
  #       quote do
  #         require Phoenix.LiveView
  #         Phoenix.LiveView.on_mount({#{module}, :mount_current_user})
  #       end
  #     end
  #   """
  # end
