defmodule Mix.Bureaucrat.Schema do
  @moduledoc false
  ...
  def new(schema_name, schema_plural, cli_attrs, opts) do
    ...
    base      = Mix.Bureaucrat.context_base(ctx_app)
    basename  = Bureaucrat.Naming.underscore(schema_name)
    module    = Module.concat([base, schema_name])
    repo      = opts[:repo] || Module.concat([base, "Repo"])
    file      = Mix.Bureaucrat.context_lib_path(ctx_app, basename <> ".ex")
    ...
    web_path = web_namespace && Bureaucrat.Naming.underscore(web_namespace)
    ...
    %Schema{
      ...
      human_singular: Bureaucrat.Naming.humanize(singular),
      human_plural: Bureaucrat.Naming.humanize(schema_plural),
  ...
  defp validate_attr!({name, :array}) do
    Mix.raise """
    Bureaucrat generators expect the type of the array to be given to #{name}:array.
    For example:

        mix bureaucrat.gen.schema Post posts settings:array:string
    """
  end
  ...
  defp partition_attrs_and_assocs(schema_module, attrs) do
    {assocs, attrs} =
      Enum.split_with(attrs, fn
        ...
        {key, :references} ->
          Mix.raise """
          Bureaucrat generators expect the table to be given to #{key}:references.
          For example:

              mix bureaucrat.gen.schema Comment comments body:text post_id:references:posts
          """
        _ -> false
      end)
    ...
    assocs =
      Enum.map(assocs, fn {key_id, {:references, source}} ->
        ...
        module = Module.concat(base ++ [Bureaucrat.Naming.camelize(key)])
